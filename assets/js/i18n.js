...
window.I18N_DICT = {
  ...
  /* ---------- CRC 计算工具（移植 crc_calculator.py） ---------- */
  'crc.doc':      { zh: 'CRC 计算工具 · Gyboy 的小站', en: "CRC Calculator · Gyboy's Site" },
  'crc.h1':       { zh: 'CRC 校验计算器', en: 'CRC Calculator' },
  'crc.subtitle': { zh: 'CRC Calculator —— 移植桌面版 CRC 计算器：99 种预设模型（CRC-3 ~ CRC-64）与全参数自定义，HEX / TEXT 双输入（支持 Reverse Word / Reverse Data 与 GBK 编码），实时输出 HEX / DEC / OCT / BIN 与位显示。',
                    en: 'CRC Calculator — desktop edition ported to the web: 99 preset models (CRC-3 … CRC-64) with fully custom parameters, HEX / TEXT input (Reverse Word / Reverse Data, GBK encoding), live HEX / DEC / OCT / BIN results and a bit view.' },
  'crc.model':    { zh: '算法模型', en: 'Model' },
  'crc.custom':   { zh: '自定义', en: 'Custom' },
  'crc.params':   { zh: 'CRC 参数（选预设自动填充，修改后自动反向匹配模型）', en: 'Parameters (auto-filled from presets; edits re-match the model automatically)' },
  'crc.width':    { zh: '位宽 WIDTH', en: 'Width WIDTH' },
  ...
  'crc.check':    { zh: 'Check 校验值', en: 'Check value' },
  ...
  'crc.hex':      { zh: 'HEX', en: 'HEX' },
  'crc.ascii':    { zh: 'TEXT', en: 'TEXT' },
  'crc.dataLabel':{ zh: '输入数据', en: 'Data' },
  'crc.hexHint':  { zh: '（十六进制字节，空格分隔，单段奇数个字符自动在前补 0，例：31 32 33 34）',
                    en: ' (hex bytes, space-separated; odd-length groups are left-padded, e.g. 31 32 33 34)' },
  'crc.asciiHint':{ zh: '（按所选编码逐字节计算）', en: ' (encoded byte-wise with the selected charset)' },
  'crc.encoding': { zh: '编码', en: 'Encoding' },
  'crc.revWord':  { zh: 'Reverse Word（每 4 字节反序）', en: 'Reverse Word (swap each 4 bytes)' },
  'crc.revData':  { zh: 'Reverse Data（整体倒序）', en: 'Reverse Data (reverse all bytes)' },
  'crc.bytes':    { zh: '字节数', en: 'Bytes' },
  'crc.result':   { zh: '计算结果', en: 'Result' },
  'crc.bitGrid':  { zh: '位显示（bit 0 ~ 63）', en: 'Bit view (bit 0 – 63)' },
  'crc.f1': { zh: '算法为 MSB 先行逐位计算：寄存器左移，数据字节（REFIN 时先按位反转）从最高位并入，最高位与数据位异或决定是否异或 POLY。',
              en: 'MSB-first bitwise algorithm: the register shifts left, each data byte (bit-reversed when REFIN) enters from the MSB, and the XOR of the top bit with the data bit decides whether POLY is applied.' },
  'crc.f2': { zh: 'REFOUT 为真时最终寄存器按位宽整体反转，再与 XOROUT 异或；空数据输入时结果为 0（与源实现一致）。',
              en: 'With REFOUT the final register is reflected over the width, then XOR-ed with XOROUT; empty input yields 0 (as in the reference implementation).' },
  'crc.f3': { zh: 'Check 值是当前参数对标准测试串 "123456789" 的计算结果，用于核对参数是否与某一标准模型一致。',
              en: 'The Check value is the result for the standard test string "123456789" — use it to verify a configuration against a known model.' },
  'crc.f4': { zh: '位宽 1 ~ 64 任意可选；位宽超过 32 时自动使用大整数运算，结果与查表实现一致。',
              en: 'Any width from 1 to 64; widths above 32 automatically use big-integer arithmetic, matching table-driven implementations.' },
  'crc.f5': { zh: '修改任意参数后会自动反向匹配预设模型；无匹配时类型自动切换为"自定义"。',
              en: 'Editing any parameter re-matches the preset list; when nothing matches, the type switches to Custom automatically.' },
  'crc.t1': { zh: 'HEX 输入按空格分段，单段奇数个字符自动在前补 0（"123" 视作 "0123"）；复制网页数据时注意首尾不可见字符。',
              en: 'HEX input is split on spaces; odd-length groups get a leading zero ("123" reads as "0123"). Watch for invisible characters when copying from web pages.' },
  'crc.t2': { zh: 'Reverse Word 将每 4 字节一组倒序（32 位字小端序常用），Reverse Data 将整个数据序列倒序，两者可叠加。',
  ...
  /* ---------- 分压电阻计算器（DCDC 反馈） ---------- */
  'vd.doc':      { zh: '分压电阻计算器 · Gyboy 的小站', en: "Voltage Divider Calculator · Gyboy's Site" },
  'vd.h1':       { zh: '分压电阻计算器', en: 'Voltage Divider Calculator' },
  'vd.subtitle': { zh: 'DC-DC 反馈分压设计：输入基准电压 V_REF 与目标输出 V_out，自动计算分压比并从 E96（±1%）/ E24（±5%）标准阻值中推荐最优电阻组合，附实际输出误差与分压电流。',
                   en: 'DC-DC feedback divider design: enter V_REF and the target V_out to compute the divider ratio and get the best resistor pairs from the E96 (±1%) / E24 (±5%) standard series, with actual output error and divider current.' },
  'vd.vref':     { zh: '基准电压 V_REF', en: 'Reference voltage V_REF' },
  'vd.vout':     { zh: '目标输出 V_out', en: 'Target output V_out' },
  'vd.r2min':    { zh: 'R₂ 取值下限', en: 'R₂ minimum' },
  'vd.r2max':    { zh: 'R₂ 取值上限', en: 'R₂ maximum' },
  'vd.unitV':    { zh: '（V）', en: ' (V)' },
  'vd.unitK':    { zh: '（kΩ）', en: ' (kΩ)' },
  'vd.schemTitle': { zh: '分压拓扑', en: 'Divider topology' },
  'vd.svg.voutName': { zh: '输出电压 V_out', en: 'Output V_out' },
  'vd.svg.fb':   { zh: 'FB / V_REF 节点', en: 'FB / V_REF node' },
  'vd.svg.r1':   { zh: 'R₁ · 上分压', en: 'R₁ · high-side' },
  'vd.svg.r2':   { zh: 'R₂ · 下分压', en: 'R₂ · low-side' },
  'vd.svg.actual': { zh: '实际输出', en: 'Actual output' },
  'vd.svg.formula': { zh: 'V_out = V_REF × (1 + R₁/R₂)', en: 'V_out = V_REF × (1 + R₁/R₂)' },
  'vd.cards.r1': { zh: 'R₁ 推荐（上分压）', en: 'R₁ pick (high-side)' },
  'vd.cards.r2': { zh: 'R₂ 推荐（下分压）', en: 'R₂ pick (low-side)' },
  'vd.cards.vact': { zh: '实际输出 V_out', en: 'Actual V_out' },
  'vd.cards.err': { zh: '输出误差', en: 'Output error' },
  'vd.tbl96':    { zh: 'E96（±1%）推荐组合', en: 'E96 (±1%) picks' },
  'vd.tbl24':    { zh: 'E24（±5%）推荐组合', en: 'E24 (±5%) picks' },
  'vd.tblR1':    { zh: 'R₁（标准值）', en: 'R₁ (std)' },
  'vd.tblR2':    { zh: 'R₂（标准值）', en: 'R₂ (std)' },
  'vd.tblIdeal': { zh: 'R₁ 理想值', en: 'R₁ ideal' },
  'vd.tblVout':  { zh: '实际 V_out', en: 'Actual V_out' },
  'vd.tblErr':   { zh: '误差', en: 'Error' },
  'vd.tblI':     { zh: '分压电流', en: 'Divider current' },
  'vd.bestTag':  { zh: '最优', en: 'Best' },
  'vd.f1': { zh: '反馈分压关系：<code>V_out = V_REF × (1 + R₁/R₂)</code>；给定 V_REF 与目标 V_out，理想比值 <code>R₁/R₂ = V_out/V_REF − 1</code>。',
             en: 'Feedback divider relation: <code>V_out = V_REF × (1 + R₁/R₂)</code>; given V_REF and target V_out, the ideal ratio is <code>R₁/R₂ = V_out/V_REF − 1</code>.' },
  'vd.f2': { zh: '推荐算法：在 R₂ 取值范围内逐个扫描标准阻值，为每个 R₂ 计算理想 R₁ 并取最接近的标准值，再按实际输出误差排序；每个数量级给出误差最小的一组。',
             en: 'Algorithm: sweep standard R₂ values across the chosen range, pair each with the nearest standard R₁, sort by actual output error; the best pair per decade of R₂ is listed.' },
  'vd.f3': { zh: 'E24 为 ±5% 系列，E96 为 ±1% 系列；±1% 电阻通常可实现 <0.5% 的输出误差，±5% 一般在百分之几量级。',
             en: 'E24 is the ±5% series and E96 the ±1% series; ±1% parts typically achieve <0.5% output error, ±5% parts a few percent.' },
  'vd.f4': { zh: '分压电流 <code>I = V_out/(R₁+R₂)</code>：阻值越大静态损耗越小，但需远大于芯片 FB 引脚漏电流，否则精度下降。',
             en: 'Divider current <code>I = V_out/(R₁+R₂)</code>: larger resistors waste less power but must stay well above the FB pin leakage current, or accuracy suffers.' },
  'vd.f5': { zh: '总误差还包含 V_REF 精度与电阻温漂，严格设计时请按最坏情况合成评估。',
             en: 'Total error also includes V_REF accuracy and resistor tempco — combine worst cases for a rigorous design.' },
  'vd.t1': { zh: 'FB 走线应短而干净，远离开关节点与电感；必要时在 R₁ 上并联 10~100 pF 前馈电容改善瞬态与纹波。',
             en: 'Keep the FB trace short and clean, away from switching nodes and inductors; a 10–100 pF feedforward cap across R₁ can improve transient response.' },
  'vd.t2': { zh: '低功耗设计优先选总阻更大的组合（分压电流 µA 级），先确认芯片手册允许的 FB 漏电流与最小负载。',
             en: 'For low-power designs prefer larger total resistance (µA-level divider current) after checking the datasheet limits on FB leakage and minimum load.' },
  'vd.t3': { zh: '需要输出可调时，可将 R₁ 换成电位器，或在 R₁ 两端并联微调电阻；校准以实际 V_REF 为准。',
             en: 'For adjustable output, replace R₁ with a potentiometer or parallel a trim resistor; calibrate against the actual V_REF.' },
  'vd.msg.positive': { zh: 'V_REF 与 V_out 需为大于 0 的数值。', en: 'V_REF and V_out must be positive numbers.' },
  'vd.msg.refBelow': { zh: 'V_REF 必须小于目标 V_out（两者相等时 R₁ = 0，应将 FB 直接接到 V_out）。',
                       en: 'V_REF must be lower than the target V_out (if equal, R₁ = 0 — tie FB directly to V_out).' },
  'vd.msg.range': { zh: 'R₂ 取值范围无效：需满足 0 < 下限 ≤ 上限。', en: 'Invalid R₂ range: require 0 < min ≤ max.' }
};
